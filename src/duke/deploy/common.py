...
class Config(object):
    def __init__(self, buildout_dir):
        self.cfg_path = os.path.join(buildout_dir, '.duke/.duke.deploy.cfg')
        ...
        if self._config.has_option('duke.deploy', 'rewrites'):
            for rewrite in self._config.get('duke.deploy', 'rewrites').split('\n'):
    ...
    def save(self):
        ...
        if not self._config.has_section('duke.deploy'):
            self._config.add_section('duke.deploy')
        self._config.set('duke.deploy', 'rewrites', "\n".join(" ".join(x) for x in self.rewrites))
